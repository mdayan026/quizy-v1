...
export default function QuestionSlider ({ changueCurrent, setTime, getAnotherQuestions }) {
	...
	if (loadingInfinity || !questions) {
		return <div className='flex h-32 md:h-[6.5rem] items-center justify-center rounded-md bg-[#1404f6] px-5 mdpx-10: py-6 text-white text-xl font-semibold'>
				...
				questions.map((question, i) => {
					return (
						<div key={question.correctAnswer + i} className={`transition-all duration-500 ${i === 0 ? '' : 'slide-right'} absolute text-center w-full`} id={'question-' + (i + 1)}>
							...
							<p className='rounded-md h-32 md:h-[6.5rem] flex justify-center items-center bg-[#1404f6] px-5 md:px-10 py-6 text-white text-xl font-semibold mb-3'>
								{question.question}
							</p>
